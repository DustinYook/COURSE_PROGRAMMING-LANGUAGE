/*------------ ie2.c -------------------------------------------------------------
Program: Homework1_ie2.c
Description: This program is designed to express the leftmost parse tree
Author: YOOK DONGHYUN
Student ID: 12131819
User ID: s12131819
Grade: Junior
Department: Asia Pacific School of Logistics
E-mail: yookdonny3@naver.com
Creation: 29 NOV 2017
Modification: 29 NOV 2017
...
struct node* factor(void)
{
	...
	struct node* Factor;
	c = nextChar();
	if (c == (char)(endOfFile) || c == (char)(endOfLine))
		Factor = NULL;
	else if (c == '-')
...
struct node* term(void)
{
	...
	Term = factor();
LABEL1:
	if (c != endOfLine && c != endOfFile)
	{
		c = nextChar();
		if (c == '*' || c == '/')
			Term = binopNode(c, Term, factor());
		else
		{
			ungetChar(c);
			goto LABEL2;
